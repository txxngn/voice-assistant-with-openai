...
def speech_to_text_route():
    ...
    audio_binary = request.data # Get the user's speech
...
def process_prompt_route():
    ...
    # Call text_to_speech function to convert OpenAI Api's reponse to speech
    openai_response_speech = text_to_speech(openai_response_text, voice)
    
    # convert openai_response_speech to base64 string so it can be sent back in the JSON response
    #openai_response_speech is a type of audio data-> can't send it to json as it only store textual data
    #-> base64 encoding - convert any binary data to textual representationby encoding the data in base64 format.
